...
public class CheckValidity extends Action {
	...
	public String doOnDocument(Document doc) {
		...
		String res="";
		if (!de.mxro.utils.Utils.getExtension(doc.getFilename()).equals("xml")) {
			...
			if (this.repair) {
				...
				final File newFile = newRoot.forceFile(de.mxro.utils.Utils.removeExtension(doc.getFilename())+".xml");
		...
		try {
			try {
				final LinnkAlias la = LinnkAlias.fromStream(new FileInputStream(new java.io.File(de.mxro.utils.Utils.removeExtension(((NodeDocument) doc).getFile().getURI().getFile().getAbsolutePath())+".linnk")));
				if (!la.isValid()) {
					de.mxro.utils.log.UserError.singelton.log(docId+ ": Linnk Alias bad ");
					if (this.repair) {
						final LinnkAlias laNew = new LinnkAlias(doc.getFilename());
						laNew.toStream(new FileOutputStream(new java.io.File(de.mxro.utils.Utils.removeExtension(((NodeDocument) doc).getFile().getURI().getFile().getAbsolutePath())+".linnk")));
